...
async function handleCreateBooking(event) {
    try {
        ...
        const { roomId, startDate, endDate, dogs, roomTitle, pricePerNight } = room || {};
        ...
        if (checkInDate < new Date()) {
            return corsErrorResponse(400, 'Check-in date cannot be in the past', origin);
        }
        ...
        if (!available) {
            return corsErrorResponse(400, 'Room is not available for selected dates', origin);
        }
